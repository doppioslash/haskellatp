...
> import Formulas(Formula(..),Clauses)
...
> oneLiteralRule :: Ord a => Clauses a -> Maybe (Clauses a)
...
> affirmativeNegativeRule :: Ord a => Clauses a -> Maybe (Clauses a)
...
> findBlowup :: Ord a => Clauses a -> Formula a -> (Int, Formula a)
> findBlowup cls l = 
...
> resolutionRule :: Ord a => Clauses a -> Clauses a
...
> resolveOn :: Ord a => Formula a -> Clauses a -> Clauses a
> resolveOn p clauses =
>   let p' = F.opp p 
...
> findCount :: Ord a => Clauses a -> Formula a -> (Int, Formula a)
> findCount cls l =
>   let m = length(filter (elem l) cls)
>       n = length(filter (elem (F.opp l)) cls) in
>   (m + n, l)
