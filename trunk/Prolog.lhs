...
> newtype Rule = Rule ([Formula Fol], Formula Fol) 

% Parsing

> instance ReadT Rule where
>   readt = parseRule
...
>      show = PP.renderStyle Print.ppStyle . ppRule
...
>       inst :: Formula Fol -> Formula Fol 
>       inst = Fol.apply $ M.fromList (zip fvs vvs) in
...
> backchain :: [Rule] -> Int -> Int -> Env -> [Formula Fol] -> Maybe Env
...
> hornprove :: Formula Fol -> IO (Env, Int)
> hornprove fm = 
...
>       tabFn n = case backchain rules' n 0 M.empty [Bot] of
>                   Nothing -> Nothing
...
> simpleprolog' :: [Rule] -> Formula Fol -> Maybe Env
> simpleprolog' rules gl = backchain rules (-1) 0 M.empty [gl]
...
> prolog :: [String] -> String -> Maybe [Formula Fol]
> prolog rules gl = prolog' (map Parse.parse rules) (Parse.parse gl)

> prolog' :: [Rule] -> Formula Fol -> Maybe [Formula Fol]
> prolog' rules gl = 
>   let mapFn env x = do t <- M.lookup x env
